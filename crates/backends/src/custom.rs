...
/// Custom steps are pure hook carriers: they have no built-in install logic
/// (everything lives in lifecycle hooks in `apps.yaml`), so every step
/// trivially converges and its hooks fire on every run. Any step id is
...
mod tests {
    ...
    fn any_step_is_a_converged_hook_carrier() {
        ...
        let t = TestEnv::new();
        let out = run("rustup", t.exec()).unwrap();
